...
def _render(tmp_path: Path, answers: dict) -> Path:
    a = tmp_path / "answers.yaml"
    a.write_text(yaml.safe_dump(answers))
    subprocess.run(
        [sys.executable, str(RENDER), "--answers", str(a), "--dest", str(tmp_path)], check=True
    )
    return tmp_path / str(answers["project_name"])
...
def test_mkdocs_documentation_scaffolded(tmp_path):
    p = _render(
        tmp_path,
        {
            "project_name": "eta",
    ...
    assert 'site_name: "eta"' in (p / "mkdocs.yml").read_text()
    assert (p / "docs" / "index.md").exists()
    assert "::: eta" in (p / "docs" / "reference.md").read_text()
    assert "mkdocs-material" in (p / "pyproject.toml").read_text()
